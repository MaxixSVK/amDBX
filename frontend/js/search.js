...
async function editUserEntry(item, type) {
    ...
    editEntryForm.classList.add('bg-white', 'p-6', 'rounded', 'shadow-lg', 'flex', 'flex-col', 'items-start', 'space-y-4', 'w-full', 'h-full', 'md:w-3/4', 'md:h-3/4', 'z-20');
    ...
    window.onclick = function (event) {
        if (event.target == document.querySelector('#edit-entry-modal > div:first-child')) {
            editEntryModal.remove();
        }
    }
}
